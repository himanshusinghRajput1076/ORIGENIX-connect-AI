...
function calculateMatchScore(input) {
    ...
    const hasDirectIndustryMatch = investor.industries.some((ind) => ind.toLowerCase() === startup.industry.toLowerCase());
    const industryScore = hasDirectIndustryMatch ? 100 : 30;
    // 2. Stage Match Score (30%)
    const hasStageMatch = investor.preferredStages.some((stg) => stg.toLowerCase() === startup.stage.toLowerCase());
    const stageScore = hasStageMatch ? 100 : 40;
    // 3. Check Size Match Score (20%)
    let checkSizeScore = 80;
    if (startup.targetRaiseAmount &&
        investor.investmentMin &&
        investor.investmentMax) {
        if (startup.targetRaiseAmount >= investor.investmentMin &&
    ...
    const totalScore = Math.round(industryScore * 0.35 +
        stageScore * 0.3 +
        checkSizeScore * 0.2 +
        signalScore * 0.15);
