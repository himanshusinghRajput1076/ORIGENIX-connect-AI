...
exports.generatePersonalizedOutreach = generatePersonalizedOutreach;
 ...
 */
async function generatePersonalizedOutreach(rawInput) {
    // 1. Strict Input Validation via Zod
    const input = shared_1.OutreachRequestSchema.parse(rawInput);
    // 2. Sanitize user inputs to prevent prompt injection / XSS
    const sanitizedPerson = (0, shared_1.sanitizeString)(input.targetPersonName);
    const sanitizedCompany = (0, shared_1.sanitizeString)(input.targetCompany);
    const sanitizedPitch = (0, shared_1.sanitizeString)(input.userPitch);
